...
![1540965584824](assets\1540965584824.png)

因为规格是跟商品分类绑定的，因此首先会展现商品分类树，并且提示你要选择商品分类，才能看到规格参数的模板。一起了解下页面的实现：

![1540965678784](assets\1540965678784.png)
...
接下来，看看Vue实例中data定义了哪些属性，对页面会产生怎样的影响：
 ...
 ![1540965731121](assets\1540965731121.png)
...
当我们点击树节点时，要将`v-dialog`打开，因此必须绑定一个点击事件：![1540965783134](assets\1540965783134.png)
...
```shell
php think make:controller api/Spec --plain
```

![1540966237809](assets\1540966237809.png)
...
增删改的作业就留给大家去完成了。页面中接口都已定义，要做的就是实现后台接口。
...
CREATE TABLE `tb_sku` (
  ...
  `stock` int(11) DEFAULT '0' COMMENT '库库',
...
这些特有属性如果排列组合，会产生12`(3*2*2)`个不同的SKU，而不同的SKU，其属性就是上面备选项中的一个。
...
现在商品表中虽然有数据，但是所有的图片信息都是无法访问的，我们需要把图片导入到项目：
...
![1542586248240](assets/1542586248240.png)

可以看出整体是一个table，然后有新增按钮。是不是跟品牌管理很像？

模板代码在分别在Goods.vue和GoodsForm.vue

## 4.2.从0开始

接下来，我们自己来实现一下，新建两个组件：Goods.vue和GoodsForm.vue
...
![1542586909988](assets/1542586909988.png)

测试效果

![1542586984463](assets/1542586984463.png)
...
![1542587057929](assets/1542587057929.png)

因为没有编写查询功能，表格一直处于loading状态。

接下来看弹窗：

![1542587078422](assets/1542587078422.png)
...
```vue
<v-data-table :headers="headers" :items="goodsList" :search="filter.search" :pagination.sync="pagination" :total-items="totalGoods"
...
- 返回结果：商品SPU的分页信息
  ...
  ```json
  {
      ...
      "items": [
          ...
          {
              "id": 3,
              "title": "三星 Galaxy C5（SM-C5000）4GB+32GB ",
              "cname": "手机/手机通讯/手机",
              "bname": "三星（SAMSUNG）"
          },
          {
              "id": 4,
              "title": "华为 麦芒5 全网通 4GB+64GB版 ",
              "cname": "手机/手机通讯/手机",
              "bname": "华为（HUAWEI）"
          },
          {
              "id": 5,
              "title": "华为 HUAWEI nova 4GB+64GB版 ",
              "cname": "手机/手机通讯/手机",
              "bname": "华为（HUAWEI）"
          },
          {
...
1. 查询tb_spu表, 得到id/title的值
2. 根据cid1/cid2/cid3 , 得到分类名称并拼接
3. 根据bid, 得到品牌名称
...
```
php think make:controller api/Goods --plain
```

![1541037459750](assets\1541037459750.png)
...
根据参数查询商品的原始参数, 相当于执行sql语句

```sql
SELECT * FROM `tb_spu` WHERE  `saleable` = 1  AND `title` LIKE '%%' LIMIT 0,10
```
...
> 注意: 在实际项目中, 这里一般会使用MySQL的存储过程, 如果不使用存储过程会执行很多的sql语句, 由于大家对存储过程不是很了解, 这里我们用熟悉的php实现这部分的逻辑
...
刷新页面，查看效果：
 ...
 ![1542616981833](assets/1542616981833.png)
